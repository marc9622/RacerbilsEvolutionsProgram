class SensorSystem {
  ...
  void displaySensors() {
    ...
    if (whiteSensorFrameCount>0) {
      ...
      fill(0, clockWiseRotationFrameCounter * 3, 0);
    }
  ...
  void updateSensorsignals(PVector pos, PVector vel) {
    ...
    if (lastGreenDetection && !currentGreenDetection && clockWiseRotationFrameCounter > 0) {  //sidst grønt - nu ikke -vi har passeret målstregen 
      int newLapTimeInFrames = frameCount - lastTimeInFrames; //LAPTIME BEREGNES - frames nu - frames sidst
      if(newLapTimeInFrames < lapTimeInFrames && newLapTimeInFrames > 10)
        lapTimeInFrames = newLapTimeInFrames;
    ...
    clockWiseRotationFrameCounter = deltaHeading>0 ? clockWiseRotationFrameCounter + 1 : clockWiseRotationFrameCounter - 5;
    if(clockWiseRotationFrameCounter > 50)
      clockWiseRotationFrameCounter = 50;
